...
class BacktestService:
    ...
    def run(self, strategy: Strategy, start: dt.date | str, end: dt.date | str) -> dict:
        ...
        parameters = {
            ...
            "entry_rule": "first tradable open from D4, maximum three trading days",
            "exit_rule": "next open after full fill, otherwise D+10 close",
        }
        ...
        counts = {"candidates": 0, "confirmed": 0, "invalidated": 0, "indeterminate": 0, "unfilled_entry": 0}
        ...
        try:
            ...
            for signal_date in days:
                ...
                for symbol in day_universe.symbols:
                    ...
                    counts["candidates"] += 1
                    confirmation_days = [
                        self.calendar.nth_trading_day_after(signal_date, offset) for offset in range(1, 4)
                    ]
                    confirmation_cutoff = confirmation_days[-1]
    ...
    def _build_event(
        ...
        for horizon in strategy_config["backtest_horizons"]:
            ...
            if horizon_bar is None:
                horizons[str(horizon)] = {"gross_return": None, "net_return": None}
            else:
        ...
        last_horizon = max(int(value) for value in strategy_config["backtest_horizons"])
    ...
    def _metrics(events: list[dict[str, Any]], counts: dict[str, int]) -> dict[str, Any]:
        ...
        gross = pd.Series([event["gross_return"] for event in filled], dtype=float)
        ...
        for key in horizon_keys:
            ...
            horizons[key] = {"gross": _series_metrics(gross), "net": _series_metrics(net_horizon)}
        phase_metrics = {}
        ...
        observed_candidates = counts["confirmed"] + counts["invalidated"] + counts["indeterminate"]
        return {
            **counts,
            "events": len(events),
            "filled_entries": len(filled),
            "gross_return": _series_metrics(gross),
            "net_return": _series_metrics(net),
            "horizon_returns": horizons,
            "full_fill_rate_d1_d3": (counts["invalidated"] / observed_candidates if observed_candidates else None),
